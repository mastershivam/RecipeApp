...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (requesterErr || !requester || requester.status !== "accepted" || requester.role !== "owner") {
      res.statusCode = 403;
      res.end("Only group owners can change roles.");
      return;
    }
    ...
    if (target.role === "owner") {
      res.statusCode = 400;
      res.end("Owner role cannot be changed.");
      return;
    }
    ...
    res.statusCode = 500;
    res.end(err instanceof Error ? err.message : "Failed to update role.");
  }
}
